...
### Changed
- **HF auth: public BAAI must not depend on HF_TOKEN:** `HTTPGetToFile` treats `HF_TOKEN` / `HUGGING_FACE_HUB_TOKEN` as optional. BAAI/bge-small-en-v1.5 is **public (no auth)**. If a token is set and Hugging Face returns 401/403, retry once **without** Authorization so a bad/expired token cannot block the public ONNX. 404 is **not** retried (`KnightsAnalytics/bge-small-en-v1.5` is a missing repo, not an auth miss). `OPENAI_API_KEY` is required only for generate/judge. Kernel-only · not Memory GA · dual_write OFF.
- **QA reader uses full retrieve-k:** `longmemeval_qa_generate.py` default `LONGMEMEVAL_READER_K=0` feeds **all** retrieved snippets to the reader (was `memories[:15]` while retrieve k=40). Multi-session 0/2 on n=12 was retrieve/reader context, not embedder. Not official V1. Kernel-only · not Memory GA · dual_write OFF.
- **BGE ONNX from BAAI when KnightsAnalytics 404:** `KnightsAnalytics/bge-small-en-v1.5` is not a published HF repo (auth 404). The download helper reshapes public `BAAI/bge-small-en-v1.5` `onnx/model.onnx` into hugot layout `testdata/models/BAAI_bge-small-en-v1.5/` (gitignored; not vendored). MiniLM remains the local unpublished fallback. TTFH cost-max is still hash. Kernel-only · not Memory GA · dual_write OFF.
- **ONNX download helper fail-soft:** Hugging Face 401/404 for `KnightsAnalytics/bge-small-en-v1.5` no longer `os.Exit(1)`. Helper prints the error on stderr, then MiniLM fallback `testdata/models/KnightsAnalytics_all-MiniLM-L6-v2` when present (not the official V1 BGE pin; hash-overlap unpublished), else empty stdout and TTFH cost-max hash (no ONNX required). Mkdir failures still exit 1. Official V1 pin stays BGE-small-en-v1.5; this repo does not vendor BGE. Kernel-only · not Memory GA · dual_write OFF.
- **LongMemEval BGE ONNX retry (SKIP):** `KnightsAnalytics/bge-small-en-v1.5` hugot download still **401**; authenticated Hugging Face lookup **404**. Official V1 embed card not filled. MiniLM internal unpublished mixed card unchanged. **No README number.** Kernel-only · not Memory GA · dual_write OFF.
...
### Added
- **Locked mixed n=60 ID list:** [`testdata/longmemeval_baseline_ids_n60.json`](testdata/longmemeval_baseline_ids_n60.json) (`sample_mixed` 10 of each type; first 12 IDs match the n=12 lock). `make longmemeval-baseline` still defaults to n=12. n=60 is the next scale, not official V1 (not 500). Not a README number. Kernel-only · not Memory GA · dual_write OFF.
...
- **TTFH air-gap / no-PULSE worked miss:** [`docs/TTFH.md`](docs/TTFH.md) Host path shows `/memory digest --require-sources mesh,private` after three private RCA ingest turns printing an explicit miss (no mesh-class receipt). That miss is success for the flag. Do not stamp mesh on local overlay. Catalog/grant/external never satisfy cite-both. This transcript is docs, not a laptop PULSE run — **E-G1 is not closed**. dual_write OFF · not Memory GA · public MIT ≠ GA.
- **Operator TTFH page:** [`docs/TTFH.md`](docs/TTFH.md) is the walking skeleton (three RCA-shaped turns, same-process retrieve, facts-as-of, print `source_hint`). Points at `go run ./examples/ttfh_rca`. Optional host path: iomesh-tui **v1.3.6** + iomesh-memory-mcp **v0.4.1** — `/memory ingest` + `/memory digest --require-sources mesh,private` (cite-both or explicit miss). Cost-max: hash embedder, no Qdrant, no cloud palace. **E-G1** (real laptop PULSE+3 RCA+cite-both-or-miss) is **not** satisfied by this page or a unit test. dual_write OFF · not Memory GA · public MIT ≠ GA.
- **Two-process writer probe:** `scripts/two_process_writer_probe.sh` and `cmd/two-process-writer-probe` run two OS processes against one palace root (distinct entry IDs plus one colliding ID) and print last-write-wins / whether `entity-graph.json` and `event-time.json` remain valid JSON. `TestTwoProcessWriterProbe` is skipped unless `MEMORY_TWO_PROCESS_PROBE=1`. Multi-process writers remain **unsupported**; flock is not shipped. Probe ≠ lock · not Memory GA · dual_write OFF.
- **LongMemEval internal MiniLM mixed-run card:** `docs/LONGMEMEVAL.md` Internal run log filled from a mixed-type V1-shaped generate+judge sample. Embed mode is in-tree ONNX MiniLM-L6-v2 (local path when BGE-small-en-v1.5 is unavailable; HF download may 401) — **not** the official V1 BGE pin. Official judge pin `gpt-4o-2024-08-06` (`scripts/longmemeval_judge.sh` maps that pin to upstream `evaluate_qa.py` zoo key `gpt-4o`). Makefile/script comments record the MiniLM fallback. **INTERNAL unpublished · not Memory GA · not a README number · dual_write OFF.**
