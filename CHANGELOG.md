...
- **Temporal roadmap:** [`docs/temporal-memory-kernel-roadmap.md`](docs/temporal-memory-kernel-roadmap.md) records official V1 first run (**388/500** BGE mixed n=500, INTERNAL unpublished, [#140](https://github.com/iome-sh/memory/pull/140)) and the next phased rollout: T6+ weeks/months text-date delta **shipped** ([#141](https://github.com/iome-sh/memory/pull/141) `7deafa5`); latest-value clip **shipped** ([#143](https://github.com/iome-sh/memory/pull/143) `5d3aca5`); restaurant tried-count **shipped** ([#146](https://github.com/iome-sh/memory/pull/146) `89c1dc0`); Wave K hash-only n=12 remesure **11/12** (`6aeb4375` **PASS**); official V1 run 2 (same pin `9bee542`) **in flight** — do not invent a second-run score; MiniLM/BGE not this remesure; KU `852ce960` remesure TBD (n=60, not n=12) / clothes reader residual; T2 btree / T3 edges / T5 Qwen3 / T8 dual-clock still parked. Shipped T1–T7 / T4-perf / T6 days-only / T7+ restaurants unchanged. btree / typed edges / Qwen3 default still gated.
