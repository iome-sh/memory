...
### Added
- **T1 count-evidence action clusters:** `ExtractAtomicFacts` names clothing errands (dry-clean, pick-up/return of boot/blazer/Zara). `AssembleCountEvidence` diversifies by action+object so compound “return and pick them up” is two bullets and dry-clean stays on pick/return clothing counts. Not a README number.
- **LongMemEval batch retrieve scoring:** `cmd/longmemeval-server` loads the ONNX embedder the same way as `cmd/longmemeval-bench` and sets `BatchEmbeddingFunc`, so retrieve scores palace candidates in one forward pass when vectors are not persisted (library default). Hash fallback when the model is missing. Optional `LONGMEMEVAL_PERSIST_EMBEDDINGS=1` persists ONNX vectors only (never hash). Health `embed_mode` still reports hash / onnx-minilm-l6-v2 / onnx-bge-small-en-v1.5. Not a README number.
- **T1 count-query cross-session fact assembly:** count queries union matching `turn_fact` / `fact_augmented` / `atomic_fact` children from the session/`conv:` set (not only keyword hits), rank named-pattern facts above fallback chatter, then diversify+Limit. `AssembleCountEvidence` lists unique snippets; LongMemEval retrieve prepends a synthetic hit (not persisted). Not a README number. Next: remesure n=12 after this + #110.
- **T1 count-query fact ranking + led-project extract:** count queries rank `turn_fact` children by keyword overlap with the query (not just “all facts first”). `ExtractAtomicFacts` matches `led`/`leading` + project/team/analysis. Locked n=12 after fact promotion (`a25a883`): hash/BGE **11/12**, `multi-session` **1/2**; MiniLM still 10/12 / 0/2. Not a README number.
- **T1 count-query fact promotion:** `how many` / `how much` search puts `turn_fact` / `fact_augmented` children ahead of haystack chatter before Limit.
- **T1 n=12 remeasure (docs):** [`docs/LONGMEMEVAL_BASELINE.md`](docs/LONGMEMEVAL_BASELINE.md) waves A–C: pre-T1 9–10/12; T1 retrieve still 0/2 multi-session; fact promotion hash/BGE **11/12** (`multi-session` 1/2). MiniLM still 10/12 / 0/2. Not a README number.
- **T1 multi-session retrieve:** `SessionIDs` any-of filter on search / list / facts-as-of / multi-hop. `conv:<id>` tags group inner haystack sessions so retrieve with the conversation id still finds them. Search diversifies distinct `SessionID`s before Limit. LongMemEval ingest keeps per-turn `session_id` and stamps `conv:<conv_id>`. Count questions are not calendar windows.
