...
| FS scan **O(n)** | First list / `InvalidateMetaIndex` / stamp-mismatch still O(n) · clean Write/unlink skip the walk |
| Best-effort meta index (s1066) | Kernel optional cache · plus durable snapshot (#44) · incremental patch (#63) · **not** invent index green |
| Durable snapshot `indexes/event-time.json` (#44) | **Shipped** best-effort · FS Palace source of truth |
| Incremental Write/unlink patch (#63) | **Shipped** · patches clean in-memory + optional durable · not btree |
| Btree/tag event-time index residual | **Residual** · not shipped · residual ≠ invent index green |
...
| Incremental Write/unlink patch | **Shipped** (#63) | Clean index patched in place; dirty/unbuilt still rebuilds from FS |
...
| **#63** | Incremental Write/unlink meta-index patch · FS SoT · btree/tag residual |
