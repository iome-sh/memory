...
|----------|----------|------------|
| Clothes gold 3, reader sums 2 | Waves G–I: 3 numbered clusters in retrieve | **Numbered `1. 2. 3.` bullets shipped** ([#127](https://github.com/iome-sh/memory/pull/127) `a4c0445`). Wave I: **miss all three**. Wave J hash: **FAIL** (reader summed 2). n=60 `5154a76`: MiniLM **PASS**; hash/BGE still summed 2. Does not invent “the answer is 3”. |
...
n=60 `5154a76` remesure: kits/hours/plants **pass all three**. Restaurant remesure after [#136](https://github.com/iome-sh/memory/pull/136): Wave J hash n=12 kernel `5d3aca5` `6aeb4375` still **FAIL** (3 vs gold 4). MiniLM/BGE **not this remesure**. n=60 `5154a76` is **before** #136.
...
3. **Restaurant remesure after [#136](https://github.com/iome-sh/memory/pull/136) — hash n=12 still FAIL.** Wave J kernel `5d3aca5`: `6aeb4375` hyp **three** vs gold **four**. MiniLM/BGE **not this remesure** — do not invent those scores. n=60 `5154a76` is before #136.
...
5. **Clothes reader residual — park more assembly.** Wave I numbered `1. 2. 3.` + N=3 in retrieve; reader summed 2. Wave J hash `0a995998` still **FAIL**. Do not start Qwen3 or dual-clock to chase it.
