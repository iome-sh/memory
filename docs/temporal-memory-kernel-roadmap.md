...
| n=60 hash-only after #143+#146+#150+#152 | `77b2839` | 52/60 MS 9/10 | SKIP | SKIP | Current `origin/main` at remesure. Complete 60/60. KU `852ce960` **FAIL** $350k (clip lists `$400,000` first). KU `6aeb4375` **PASS** (hyp four). Clothes `0a995998` FAIL. `08f4fc43` FAIL (hyp 30 days, judge). Kits/hours/plants PASS. vs `5154a76` hash 49/60. MiniLM/BGE **not run**. |
...
| Unique-entity n=60 (kits 5, hours 15, plants) | n=60 v1.5.12 **before** [#119](https://github.com/iome-sh/memory/pull/119) | **T7 shipped** ([#132](https://github.com/iome-sh/memory/pull/132) `e4dcb73`). n=60 `5154a76`: kits `gpt4_59c863d7` / hours `aae3761f` / plants `3a704032` **pass all three**. n=60 hash-only `77b2839`: kits/hours/plants **PASS**. |
| Days-between TR | n=60 `08f4fc43` / `2a1811e2` / `2c63a862` | **T6 shipped** ([#129](https://github.com/iome-sh/memory/pull/129) `625a772`). n=60 `5154a76`: `2a1811e2` / `2c63a862` **pass all three**; `08f4fc43` **miss all three**. n=60 hash-only `77b2839`: `2a1811e2` / `2c63a862` **PASS**; `08f4fc43` **FAIL** (hyp 30 days; retrieve `text dates 30 days apart (January 2nd → February 1st)`; judge). Official V1 run 2 (pin `9bee542`): `08f4fc43` **FAIL** despite hyp 30 days (judge). Days-only. Does not invent gold. |
...
n=60 `5154a76` remesure: `2a1811e2` / `2c63a862` pass all three; `08f4fc43` miss all three. n=60 hash-only `77b2839`: `2a1811e2` / `2c63a862` **PASS**; `08f4fc43` **FAIL** (hyp 30 days; `text dates 30 days apart (January 2nd → February 1st)`; judge).
...
7. **Clothes reader residual — park more assembly.** Wave I numbered `1. 2. 3.` + N=3 in retrieve; reader summed 2. Wave J/K/L/M/N hash `0a995998` still **FAIL**. n=60 hash-only `77b2839` `0a995998` still **FAIL**. Do not start Qwen3 or dual-clock to chase it.
8. **Latest-value clip shipped; KU `852ce960` remesure hash-only `77b2839` still FAIL.** [#143](https://github.com/iome-sh/memory/pull/143) `5d3aca5` `clipKeepingAmount` keeps `$400,000` in long snippets (Nov 30 turn was prefix-clipped at 280 chars). Stale `$350,000` still listed. Does **not** NLP-supersede. n=60 `5154a76` / official V1 first and run 2 on pin `9bee542` (before #143) still $350k. n=60 hash-only current kernel `77b2839` (after #143): still **FAIL** (reader `$350,000`; retrieve lists `$400,000` first then stale `$350,000`). MiniLM/BGE **not this remesure**.
