...
| Official V1 TR order extrema | First run TR **83/133 (0.624)** (`9bee542`) | **T6++ shipped** ([#147](https://github.com/iome-sh/memory/pull/147) `298584c`). `text dates earliest: PHRASE · latest: PHRASE` from parsed text times. Not ingest `Timestamp`. Official V1 remesure **TBD**. Do not invent gold. |
...
3. **Restaurant tried-count — shipped.** [#146](https://github.com/iome-sh/memory/pull/146) `89c1dc0`: latest-first “tried N” self-reports; cuisine+BBQ dishes are not venues; stop-token `if`. Does not invent gold 4. Wave J hash n=12 kernel `5d3aca5` `6aeb4375` still **FAIL** (3 vs 4) is **before** this PR. Wave K hash n=12 kernel `89c1dc0` `6aeb4375` **PASS** (hyp four) ([#148](https://github.com/iome-sh/memory/pull/148) `fe1e66f`). MiniLM/BGE **not** Wave J/K — do not invent those scores. n=60 `5154a76` is before #136.
4. **T6++ temporal-order extrema — shipped.** [#147](https://github.com/iome-sh/memory/pull/147) `298584c`: `text dates earliest: PHRASE · latest: PHRASE` from parsed text times. Dated-span keeps T6/T6+ deltas. Not ingest `Timestamp`. Do not invent gold. Official V1 remesure **TBD**.
...
6. **Clothes reader residual — park more assembly.** Wave I numbered `1. 2. 3.` + N=3 in retrieve; reader summed 2. Wave J/K hash `0a995998` still **FAIL**. Do not start Qwen3 or dual-clock to chase it.
