...
| J #136+#143 hash-only | `5d3aca5` | 10/12 1/2 | SKIP | SKIP | After restaurant clusters [#136](https://github.com/iome-sh/memory/pull/136) + clip [#143](https://github.com/iome-sh/memory/pull/143). Clothes `0a995998` FAIL (reader 2); projects `6d550036` PASS; KU `6aeb4375` still 3 vs 4. MiniLM/BGE **not run**. |
...
Do not start Qwen3 or a dual-clock KG to chase n=12 clothes (that miss is reader assembly). btree / typed edges stay gated on measured limiters, not on eval hunger. **T6+** weeks/months text-date delta is **shipped** ([#141](https://github.com/iome-sh/memory/pull/141) `7deafa5`). Latest-value clip is **shipped** ([#143](https://github.com/iome-sh/memory/pull/143) `5d3aca5`). Restaurant remesure after #146: Wave K **hash n=12 PASS** (`89c1dc0`, `6aeb4375` hyp four); Wave J was still 3 vs 4. MiniLM/BGE not this remesure. Next: KU `852ce960` remesure TBD (n=60, not n=12); official V1 **run 2** (same pin `9bee542`, **in flight**) before any README number — do not invent a second-run score. T2 / T3 / T5 / T8 parked.
...
|----------|----------|------------|
| Clothes gold 3, reader sums 2 | Waves G–I: 3 numbered clusters in retrieve | **Numbered `1. 2. 3.` bullets shipped** ([#127](https://github.com/iome-sh/memory/pull/127) `a4c0445`). Wave I: **miss all three**. Wave J hash: **FAIL** (reader summed 2). Wave K hash: **FAIL** (reader summed 2). n=60 `5154a76`: MiniLM **PASS**; hash/BGE still summed 2. Does not invent “the answer is 3”. |
...
| KU stale amount | `852ce960` $350k vs gold $400k | **Latest-value clip shipped** ([#143](https://github.com/iome-sh/memory/pull/143) `5d3aca5`): `clipKeepingAmount` keeps `$400,000` in long snippets (Nov 30 turn was prefix-clipped at 280 chars). Stale `$350,000` still listed. Does **not** NLP-supersede. Last measured n=60 `5154a76` (and official V1 first run `9bee542`) still $350k. Remesure **TBD** (`852ce960` is n=60, not n=12). |
| KU restaurants 3 vs 4 | Wave I `6aeb4375` miss all three (gold **four**, hyp **three**) | **T7+ restaurant clusters shipped** ([#136](https://github.com/iome-sh/memory/pull/136) `0a40b0a`). **Tried-count + cuisine-BBQ not venue shipped** ([#146](https://github.com/iome-sh/memory/pull/146) `89c1dc0`). Wave J hash `5d3aca5`: **FAIL** (hyp three; `[restaurant:korean-style-bbq]` / `[restaurant:if]`). Wave K hash `89c1dc0`: **PASS** (hyp four; latest-first tried-N; no korean-style-bbq/if). Leftover `[restaurant:as]`. MiniLM/BGE **not this remesure**. n=60 `5154a76` is **before** #136: still 3 vs 4 all three. Clothing N-header unchanged. Does not invent gold 4. |
...
n=60 `5154a76` remesure: kits/hours/plants **pass all three**. Restaurant remesure after [#146](https://github.com/iome-sh/memory/pull/146): Wave K hash n=12 kernel `89c1dc0` `6aeb4375` **PASS** (hyp four). Wave J `5d3aca5` was still 3 vs 4. MiniLM/BGE **not this remesure**. n=60 `5154a76` is **before** #136.
...
3. **Restaurant remesure after [#146](https://github.com/iome-sh/memory/pull/146) — hash n=12 PASS.** Wave K kernel `89c1dc0`: `6aeb4375` hyp **four** vs gold **four**. Wave J `5d3aca5` was still 3 vs 4. MiniLM/BGE **not this remesure** — do not invent those scores. n=60 `5154a76` is before #136.
4. **Latest-value clip shipped; KU `852ce960` remesure TBD.** [#143](https://github.com/iome-sh/memory/pull/143) `5d3aca5` `clipKeepingAmount` keeps `$400,000` in long snippets (Nov 30 turn was prefix-clipped at 280 chars). Stale `$350,000` still listed. Does **not** NLP-supersede. Last measured n=60 `5154a76` / official V1 first run `9bee542` still $350k. `852ce960` is n=60, not n=12. Remesure **TBD**.
5. **Clothes reader residual — park more assembly.** Wave I numbered `1. 2. 3.` + N=3 in retrieve; reader summed 2. Wave J/K hash `0a995998` still **FAIL**. Do not start Qwen3 or dual-clock to chase it.
6. **Official V1 run 2 (same pin) in flight — do not invent a second-run score.** Same pin as first run: kernel `9bee542`, BGE-small-en-v1.5 ONNX, mixed n=500, judge `gpt-4o-2024-08-06`, reader gpt-4o-mini, session_id=conv_id. First run **388/500** is INTERNAL unpublished, not Memory GA. Run 2 is **in flight**; not a README number.
7. **T2 btree / T3 edges / T8 dual-clock / T5 Qwen3 — still parked.** btree only if rebuild is the limiter ([#131](https://github.com/iome-sh/memory/pull/131) says it is not). T3 only if gold is an expired relation. T8 only if an expired-window miss shows up. T5 Qwen3 last, consumer-driven.
