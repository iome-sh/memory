...
Wave M hash n=12 kernel `7a9b956`: n=12 temporal `gpt4_2487a7cb` is **which-first**, not ago (no `question_date` evidence line). Hash **11/12**. Official V1 remesure **TBD**. Do not invent a score.
...
6. **Next — official V1 run 2 record.** Same pin `9bee542`, BGE-small-en-v1.5 ONNX, mixed n=500, judge `gpt-4o-2024-08-06`, reader gpt-4o-mini, session_id=conv_id. Run 2 record **unpublished** — do not invent Overall Accuracy. First run **388/500** INTERNAL unpublished, not Memory GA, not a README number.
7. **Clothes reader residual — park more assembly.** Wave I numbered `1. 2. 3.` + N=3 in retrieve; reader summed 2. Wave J/K/L/M hash `0a995998` still **FAIL**. Do not start Qwen3 or dual-clock to chase it.
...
- Eval numbers in this document are **unpublished**. They are not a README number and **not Memory GA**. Official V1 **first** run is BGE mixed n=500 **388/500** (kernel `9bee542`, [#140](https://github.com/iome-sh/memory/pull/140) `865994b`) — INTERNAL unpublished; reproduce twice before public. Official V1 **run 2** (same pin) record **unpublished** — do not invent Overall Accuracy. n=12 / n=60 are an improvement baseline ≠ this V1 run.
