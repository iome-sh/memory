...
- **Residual honesty pin s1303** (closed residual-honest): K2 `ListMemoryWithOptions` / timeline path — filters before Limit shipped · **O(n)** FS scan residual-honest · full **event-time index residual** (not shipped · residual ≠ invent index green) · host `memory_timeline` / TUI s1296 mention only — see [`docs/operations/k2-event-time-index-residual.md`](docs/operations/k2-event-time-index-residual.md). Gate: `make residual-gate` or `make k2-event-time-index-residual-gate`. kernel ≠ product Memory GA · dual_write OFF · **no invent GA**.
- **Residual honesty pin s1313** (closed residual-honest): RecMem / compaction path — `AutoRecMemCompaction` shipped partial · `PerformCompaction` / `CompactionConfig` kernel primitives · host `memory_trigger_compact` **advisory** · TUI HITL peer s1311 · Compaction PASS ≠ invent Memory GA token-reduction · Phase 3 semantic refine residual if still planned — see [`docs/operations/recmem-compaction-residual.md`](docs/operations/recmem-compaction-residual.md). Gate: `make residual-gate` or `make recmem-compaction-residual-gate`. kernel ≠ product Memory GA · dual_write OFF · **no invent GA**.
